...
def undirected_label2directed_label(adj: scipy.sparse.csr_matrix, edge_pairs: List[Tuple],
    ...
    if signed_directed:
        ...
        # print(np.array(adj[edge_pairs[:, 0], edge_pairs[:, 1]]))
        # print(np.array(adj[edge_pairs[:, 0], edge_pairs[:, 1]]).flatten())
        # print(np.array(adj[edge_pairs[:, 1], edge_pairs[:, 0]]).flatten())
        # print(np.abs(np.array(adj[edge_pairs[:, 0], edge_pairs[:, 1]]).flatten()))
        ...
        new_edge_pairs = directed
        # print(new_edge_pairs)
        # print(new_edge_pairs[:, [1, 0]])
        new_edge_pairs = np.vstack([new_edge_pairs, new_edge_pairs[:, [1, 0]]])
        new_edge_pairs = np.vstack([new_edge_pairs, negative])
...
def undirected_label2directed_label_3class(adj: scipy.sparse.csr_matrix, edge_pairs: List[Tuple],
                                    task: str, directed_graph: bool = True, signed_directed: bool = False) -> Union[List, List]:
    r"""Qin Simple
...
def link_class_split_new_1split(data: torch_geometric.data.Data, size: int = None,
    ...
    # set up the observed graph and weights after splitting
    observed_edges = -np.ones((len(edges_train), 2), dtype=np.int32)
    observed_weight = np.zeros((len(edges_train), 1), dtype=np.float32)

    direct = (np.abs(A[edges_train[:, 0], edges_train[:, 1]].data) > 0).flatten()

    observed_edges[direct, 0] = edges_train[direct, 0]
    observed_edges[direct, 1] = edges_train[direct, 1]
    observed_weight[direct, 0] = np.array(
        A[edges_train[direct, 0], edges_train[direct, 1]]).flatten()
    ...
    datasets['weights'] = torch.from_numpy(
        observed_weight.flatten()).float().to(device)
    # print('----- Pesi ---')
    # print(datasets['weights'] )
    datasets['train'] = {}
    datasets['train']['edges'] = torch.from_numpy(edges_train).long().to(device)

    datasets['train']['label'] = torch.from_numpy(labels_train).long().to(device)
