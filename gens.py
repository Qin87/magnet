...
def sampling_node_source(class_num_list, prev_out_local, idx_info_local, train_idx, tau=2, max_flag=False,
    ...
    # print(prev_out_local.shape)     # torch.Size([250, 6])
    # print(prev_out_local[idx_info_local[0]].shape)      # torch.Size([20, 6]) choose noes from per class
    # print(prev_out_local[idx_info_local[0]][:,0].shape, prev_out_local[idx_info_local[0]][:,0])  # prob of nodes from per class belong to the right class. torch.Size([20]) tensor([0.2260,
    # print(idx_info_local[0])  # tensor([  1,   9,  10,  32
...
#
#     :param class_num_list:
#     :param prev_out_local:
#     :param idx_info_local:
#     :param train_idx:
#     :param tau:
#     :param max_flag:
#     :param no_mask:
#     :return:
#     """
#     # print("After warmup: ", class_num_list, np.shape(class_num_list))       # # [34, 7, 158, 341, 73, 15, 3] (7,)
#     max_num, n_cls = max(class_num_list), len(class_num_list)
#     if not max_flag:  # mean (max_flag is false)
...
#         # print(prob.shape)  # torch.Size([]) Ben Amazon-Photo raise error  torch.Size([34])
#         if prob.shape == torch.Size([]):
#             continue
#
...
#         dst_idx_all.append(dst_idx)
#         print("Before cat:", src_idx_all)   # list of tensors
#     # if len(src_idx_all) != 0:
#         src_idx_all = torch.cat(src_idx_all)
#     # if len(dst_idx_all) != 0:
#         dst_idx_all = torch.cat(dst_idx_all)
#     # print("After cat: ", src_idx_all)       # cat of tensors
#
#     return src_idx_all, dst_idx_all
...
def sampling_idx_individual_dst(class_num_list, idx_info, device):
    ...
    # print(sampling_dst_idx, dst_idx.shape, sampling_dst_idx.shape, '\n',sampling_src_idx)
