...
class SigMaNetConv_Qin(MessagePassing):
    ...
    def forward(
            ...
            x_real: torch.FloatTensor,
            x_imag: torch.FloatTensor,
            norm_real=None, norm_imag=None, edge_index=None,
            # edge_index: torch.LongTensor,
            # edge_weight: OptTensor = None,
            # lambda_max: OptTensor = None,
    ) -> torch.FloatTensor:
        ...
        return out_real, out_imag, norm_real, norm_imag, edge_index
...
class SigMaNet_node_prediction_one_laplacian(nn.Module):
    ...
    def __init__(self, num_features: int, hidden: int = 2, K: int = 1, label_dim: int = 2, \
        ...
        for _ in range(1, layer):
            chebs.append(SigMaNetConv(in_channels=hidden, out_channels=hidden, K=K, i_complex=i_complex, follow_math=follow_math, gcn=gcn, net_flow=net_flow, normalization=normalization,
                                      edge_index=edge_index, norm_real=norm_real, norm_imag=norm_imag))
...
class SigMaNet_node_prediction_one_laplacian_Qin(nn.Module):
    ...
    def __init__(self, num_features: int, K: int = 1, hidden: int = 2, label_dim: int = 2, \
        ...
        for _ in range(1, layer):
            chebs.append(SigMaNetConv_Qin(in_channels=hidden, out_channels=hidden, K=K,
                                          i_complex=i_complex, follow_math=follow_math,
                                          gcn=gcn, net_flow=net_flow, normalization=normalization))
