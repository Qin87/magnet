...
from nets.sparse_magnet import sparse_mx_to_torch_sparse_tensor, ChebNet, ChebNet_Ben
...
def test(X_real, X_img):
    ...
    # for mask in [new_train_mask, data_val_mask, data_test_mask]:
...
if args.net.startswith('Mag'):
    if args.AugDirect==0:
        ...
        Magmodel = ChebNet_Ben(X_real.size(-1), L_real, L_img, K=args.K, label_dim=n_cls, layer=args.layer,
                    activation=args.activation, num_filter=args.feat_dim, dropout=args.dropout).to(device)
    ...
    # for split in range(splits):
        ...
        for epoch in range(args.epoch):
            ...
            if args.AugDirect == 0:
                ...
                add_num = new_x.shape[0] - data_x.shape[0]
                new_train_mask = torch.ones(add_num, dtype=torch.bool, device=data_x.device)
                new_train_mask = torch.cat((data_train_mask, new_train_mask), dim=0)
                # type 1
                sampling_src_idx = sampling_src_idx.to(torch.long).to(data_y.device)  # Ben for GPU error
                ...
                if args.net == 'SymDiGCN':
                    ...
                    if epoch == 0:
                        ...
                        X_img = torch.FloatTensor(new_x).to(device)
                        X_real = torch.FloatTensor(new_x).to(device)
                    else:

                        short = X_img.shape[0] - new_x.shape[0]
                        if short > 0:
                            if short< new_x.shape[0]:
                                num_y = new_train_mask[:short].sum().item()
                                new_x = torch.cat((new_x, new_x[:short]))
                                new_train_mask = torch.cat((new_train_mask, new_train_mask[:short]))
                                new_y = torch.cat((new_y, new_y[:num_y]))
                            else:
                                repeat = short//new_x.shape[0]+1
                                relic = short % len(new_x)
                                new_x = torch.cat((torch.cat([new_x] * repeat), new_x[:relic]))
                                new_train_mask = torch.cat((torch.cat([new_train_mask] * repeat), new_train_mask[:relic]))
                                num_y = new_train_mask[:relic].sum().item()
                                new_y = torch.cat((torch.cat([new_y] * repeat), new_y[:num_y]))

                        elif short < 0:
                            num_y = new_train_mask[:X_img.shape[0]].sum().item()
                            new_x = new_x[: X_img.shape[0]]
                            new_train_mask = new_train_mask[: X_img.shape[0]]
                            new_y = new_y[:num_y]
                        else:
                            pass
            ...
            # model = Magmodel
            accs, baccs, f1s = test(X_real, X_img)
