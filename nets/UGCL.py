...
import numpy as np
import math
from utils0.hermitian import to_edge_dataset_sparse_sign
...
class UGCL_Model_Qin(torch.nn.Module):
    def __init__(self, num_hidden: int, num_proj_hidden: int,  num_label: int, tau: float = 0.5):
        super(UGCL_Model_Qin, self).__init__()
        self.tau: float = tau
    ...
    def forward(self, real, imag, q, pos_edges, neg_edges, args, size, index):
        ...
        encoder = Encoder(real.size(-1), K=args.K, label_dim=args.num_class_link,
                          layer=args.layer, num_filter=args.num_filter, dropout=args.dropout)

        return encoder(real, imag, q, pos_edges, neg_edges, args, size, index)
