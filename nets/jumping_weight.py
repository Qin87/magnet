...
class JumpingKnowledge(torch.nn.Module):
    ...
    **max pooling** (:obj:`"max"`)
    ...
    def __init__(self, mode: str, channels: Optional[int] = None,
        ...
        assert self.mode in ['cat', 'max', 'lstm', 'weighted']

        self.coefficients = torch.nn.Parameter(torch.ones(num_layers))  # Just num_layers learnable coefficients
    ...
    def forward(self, xs: List[Tensor]) -> Tensor:
        ...
        if self.mode == 'cat':
            ...
            x = torch.stack(xs, dim=-1)  # Like max mode
            coefficients = torch.softmax(self.coefficients, dim=0)  # Get normalized coefficients
            return (x * coefficients).sum(dim=-1)
        else:  # self.mode == 'lstm'
