...
Direct_dataset=( 'telegram/'  )  # 'cora_ml/'  'citeseer_npz/'  'WebKB/Cornell' 'WebKB/wisconsin'  'WebKB/texas' 'WebKB/texas' 'WebKB/wisconsin'  telegram/telegram
...
python3 main.py   --alphaDir="$alphadir"  --BN_model=1 --First_self_loop='add' --dropout=0.0 --betaDir="$betadir"    --gamaDir="$gamadir"  --dropout=0.0  --net="$net"  --layer="$layer"   --Dataset="$Didataset" > "$log_output"
             2>&1
            wait $pid
          #done
        done
