...
Direct_dataset=( 'WikipediaNetwork/chameleon' 'cora_ml/'  'citeseer_npz/' 'WikipediaNetwork/squirrel'  'dgl/pubmed'  'WikiCS/' 'telegram/'  )  # 'cora_ml/'  'citeseer_npz/'  'WebKB/Cornell' 'WebKB/wisconsin'  'WebKB/texas' 'WebKB/texas' 'WebKB/wisconsin'  telegram/telegram
...
python3 main.py   --jk='max'    --alphaDir="$alphadir"  --betaDir="$betadir" --gamaDir="$gamadir"   --use_best_hyperparams    --net="$net"  --layer="$layer"   --Dataset="$Didataset" > "$log_output"
             2>&1
            wait $pid
          done
        done
