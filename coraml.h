...
python3 main.py   --jk='max' --differ_AAt --BN_model=1  --First_self_loop='remove' --lr=0.01 --dropout=0.5 --betaDir="$betadir"   --net="$net"  --layer="$layer"   --Dataset="$Didataset" > "$log_output"
             2>&1
            wait $pid
          #done
        #done
