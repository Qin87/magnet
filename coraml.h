...
python3 main.py   --jk='max'  --BN_model=1   --lr=0.01 --dropout=0.5  --alphaDir="$alphadir" --betaDir="$betadir"   --gamaDir="$gamadir"  --dropout=0.0  --net="$net"  --layer="$layer"   --Dataset="$Didataset" > "$log_output"
             2>&1
            wait $pid
          #done
        done
