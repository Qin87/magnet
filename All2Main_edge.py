################################
# copy backup of merge of edge-prediction and All2Main.py
################################
import time
...
def train_keepAug(train_idx, edge_in, in_weight, edge_out, out_weight, SparseEdges, edge_weight,X_real, X_img, edge_Qin_in_tensor, edge_Qin_out_tensor, Sigedge_index, norm_real, norm_imag,
                  new_edge_index, new_x, new_y, new_y_train):
    global class_num_list, idx_info, prev_out
    global data_train_mask, data_val_mask, data_test_mask
    try:
        model.train()
    ...
    for split in range(splits):
        ...
        for epoch in range(args.epoch):
            if args.net.startswith('UGCL'):
                ...
                if goodAug is False:
                    val_loss, new_edge_index, new_x, new_y, new_y_train = train(train_idx, edge_in, in_weight, edge_out, out_weight, SparseEdges, edge_weight, X_real, X_img, edge_Qin_in_tensor, edge_Qin_out_tensor, Sigedge_index, norm_real,norm_imag,
                                  X_img_i, X_img_j, X_img_k,norm_imag_i, norm_imag_j, norm_imag_k, Quaedge_index)
                else:
                    val_loss, new_edge_index, new_x, new_y, new_y_train = train_keepAug(train_idx, edge_in, in_weight, edge_out, out_weight, SparseEdges, edge_weight, X_real, X_img, edge_Qin_in_tensor, edge_Qin_out_tensor, Sigedge_index, norm_real,norm_imag,
                                  X_img_i, X_img_j, X_img_k,norm_imag_i, norm_imag_j, norm_imag_k, Quaedge_index)
                accs, baccs, f1s = test()
